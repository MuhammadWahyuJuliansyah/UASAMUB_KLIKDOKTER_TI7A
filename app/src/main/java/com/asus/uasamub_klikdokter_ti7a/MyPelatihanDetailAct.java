...
public class MyPelatihanDetailAct extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference = FirebaseDatabase.getInstance().getReference().child("Dokter").child("Dokter1");
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                xnama_pelatihan.setText(dataSnapshot.child("nama_dokter").getValue().toString());
                xlokasi.setText(dataSnapshot.child("lokasi").getValue().toString());
                xtime_pelatihan.setText(dataSnapshot.child("time_dokter").getValue().toString());
                xdate_pelatihan.setText(dataSnapshot.child("date_dokter").getValue().toString());
