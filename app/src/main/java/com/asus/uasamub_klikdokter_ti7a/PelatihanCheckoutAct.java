...
public class PelatihanCheckoutAct extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference = FirebaseDatabase.getInstance().getReference().child("Dokter").child(jenis_pelatihan_baru);
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                valuehargapelatihan = Integer.valueOf(dataSnapshot.child("harga_dokter").getValue().toString());
        ...
        btn_buy_ticket.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                reference3 = FirebaseDatabase.getInstance()
                        .getReference().child("Konsul_saya")
                        .child(username_key_new).child(location_pelatihan.getText().toString() + nomor_transaksi);

                reference3.addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(DataSnapshot dataSnapshot) {
                        reference3.getRef().child("id_ticket").setValue(nama_pelatihan.getText().toString() + nomor_transaksi);
                        reference3.getRef().child("nama_dokter").setValue(nama_pelatihan.getText().toString());
                        reference3.getRef().child("lokasi").setValue(location_pelatihan.getText().toString());
                        reference3.getRef().child("ketentuan").setValue(ketentuan.getText().toString());
                        reference3.getRef().child("jumlah_dokter").setValue(valuejumlahpelatihan.toString());
                        reference3.getRef().child("date_dokter").setValue(date_pelatihan);
                        reference3.getRef().child("time_dokter").setValue(time_pelatihan);
